...
public enum ErrorSubCode {
    ...
    AUTH_API_TOKEN_SCOPE_DENIED("AUTH-009", "当前 API Token 没有调用该接口的权限。", ErrorCode.AUTH_ERROR),
    API_TOKEN_NOT_FOUND("RES-API-TOKEN-001", "API Token 不存在。", ErrorCode.RESOURCE_NOT_FOUND),
    API_TOKEN_SCOPE_REQUIRED("VAL-API-TOKEN-001", "API Token 至少需要选择一个权限范围。", ErrorCode.VALIDATION_ERROR),
    ...
    MAVEN_SETTINGS_NOT_FOUND("RES-MAVEN-001", "Maven Settings 不存在。", ErrorCode.RESOURCE_NOT_FOUND),
    PRESET_NOT_FOUND("RES-PRESET-001", "运行环境预置不存在。", ErrorCode.RESOURCE_NOT_FOUND),
    NOTIFICATION_CHANNEL_NOT_FOUND("RES-NOTIFY-001", "通知渠道不存在。", ErrorCode.RESOURCE_NOT_FOUND),
    NOTIFICATION_TEMPLATE_NOT_FOUND("RES-NOTIFY-002", "通知模板不存在。", ErrorCode.RESOURCE_NOT_FOUND),
    NOTIFICATION_WEBHOOK_CONFIG_NOT_FOUND("RES-NOTIFY-003", "Webhook 配置不存在。", ErrorCode.RESOURCE_NOT_FOUND),
    ...
    PIPELINE_NOTIFICATION_BINDING_INVALID("BIZ-PIPELINE-004", "流水线绑定了无效的通知配置。", ErrorCode.OPERATION_NOT_ALLOWED),
    PIPELINE_CONFIG_INCOMPLETE("VAL-PIPELINE-001", "流水线配置不完整。", ErrorCode.VALIDATION_ERROR),
    NOTIFICATION_TEMPLATE_BUILT_IN("BIZ-NOTIFY-001", "系统内置通知模板不允许删除。", ErrorCode.OPERATION_NOT_ALLOWED),
    NOTIFICATION_CHANNEL_WEBHOOK_REQUIRED("BIZ-NOTIFY-002", "通知配置必须绑定一个可用的 Webhook 配置。", ErrorCode.OPERATION_NOT_ALLOWED),
    NOTIFICATION_TEMPLATE_CARD_INVALID("VAL-NOTIFY-001", "通知卡片模板不是合法的 JSON。", ErrorCode.VALIDATION_ERROR),
